  ...
  const fetchUserData = useCallback(async () => { // Wrap in useCallback for useFocusEffect
    ...
    if (currentUser) {
      ...
      try {
        ...
        // In case of error, userFromFirebase remains as currentUser (Auth object)
      }
    } else {
      setUserFromFirebase(null); // No authenticated user
    }
    setLoading(false); // Set loading false after all operations
  }, []); // Empty dependency array means this useCallback function is stable

  useFocusEffect(
    useCallback(() => { // useFocusEffect also recommends wrapping its effect in useCallback
      fetchUserData();
    }, [fetchUserData]) // Rerun if fetchUserData identity changes (it won't due to its own useCallback)
  );

  // Determine the name to display with a clear priority:
  // 1. userDetails.name (from context)
  // 2. userFromFirebase.name (if it's from Firestore and has a 'name' field)
  // 3. userFromFirebase.displayName (if it's an Auth object or Firestore doc also has 'displayName')
  // 4. Fallback to "User" or "Loading..."
