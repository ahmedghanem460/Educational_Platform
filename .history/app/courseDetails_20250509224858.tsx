...
const CourseDetail = () => {
    ...
    try {
      ...
      console.error("Error fetching user profile for review:", error);
        ...
        const reviewData: Review = {
          ...
          userPhotoURL: reviewDataFromDb.userProfile?.photoURL || DEFAULT_AVATAR,
        ...
        if (user && reviewData.userId === user.uid) {
          ...
          setUserExistingReviewId(reviewData.id);
      ...
      if (!foundUserReview && user && isUpdatingReview) {
        setUserReview('');
        setRating(0);
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      if (userExistingReviewId && isUpdatingReview) {
        const reviewRef = doc(FIREBASE_DB, 'courses', title, 'reviews', userExistingReviewId);
        await setDoc(
          doc(FIREBASE_DB, 'courses', title, 'reviews', userExistingReviewId),
          reviewDataPayload,
          { merge: true }
        );
        ...
        await addDoc(
          collection(FIREBASE_DB, 'courses', title, 'reviews'),
          reviewDataPayload
        );
