...
const saveUser = async (firebaseUser: any) => { // Renamed 'user' to 'firebaseUser' for clarity
  // Data to be stored in Firestore
  const userDataForFirestore = {
    uid: firebaseUser.uid,
    name: name, // Name from the registration form input
    email: firebaseUser.email, // Email from the authenticated Firebase user object
    role: 'user',
    cartItems: [],
  ...
  try {
    ...
    // Also, update the UserDetailsContext with the same consistent data
    setUserDetails(userDataForFirestore);

    // Optionally, you can also update the Firebase Auth profile's displayName.
    // This is separate from Firestore but can be useful.
    if (name.trim() !== '' && firebaseUser.displayName !== name) {
    ...
    // Decide how to handle this error. You might want to inform the user.
    // For now, re-throwing will let the signUp's catch block handle it.
    throw error;
  }
};
...
const Register = () => {
  ...
  // Removed duplicate saveUser function to avoid shadowing issues
