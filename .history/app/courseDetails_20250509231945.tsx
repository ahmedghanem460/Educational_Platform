...
interface Review {
  id: string;
  userId: string;
  userName: string; // Name stored at the time of review
  userEmail: string;
  userPhotoURL?: string; // Photo URL stored at the time of review
  rating: number;
  review: string;
  timestamp: Timestamp;
  authorProfile?: DocumentData; // Optionally fetched live profile data
}
  ...
  const fetchReviews = async () => {
    ...
    try {
      ...
      const reviewsData = await Promise.all(querySnapshot.docs.map(async (docSnapshot) => {
        ...
        // Optionally fetch fresh profile data if you store more user details in a 'users' collection
        if (reviewDataFromDb.userId) { 
          try {
            ...
            // console.error("Error fetching user profile for review:", error); // Optional: log if needed
          }
        }
        ...
        const reviewItem: Review = {
          ...
          userName: reviewDataFromDb.userName, // Already using the name stored with the review
          userEmail: reviewDataFromDb.userEmail || '',
          userPhotoURL: reviewDataFromDb.userPhotoURL, // Already using the photo stored with the review
  ...
  const submitReview = async () => {
    ...
    try {
      const reviewDataPayload = {
        userId: user.uid,
        userName: user.displayName || 'Anonymous', // Use Firebase Auth display name
        userEmail: user.email || '',
        userPhotoURL: user.photoURL || DEFAULT_AVATAR, // Use Firebase Auth photo URL
        rating,
  ...
  const ReviewSection = () => (
    <View style={styles.reviewSection}>
      ...
      {isBought && user && (
        <View style={styles.reviewForm}>
          <View style={styles.reviewerInfo}>
            <Image
              source={ // Use current user's photoURL from Firebase Auth
                user.photoURL
                  ? { uri: user.photoURL }
                  : { uri: DEFAULT_AVATAR }
              }
            ...
            <Text style={styles.reviewerName}>
              {user.displayName || 'Anonymous'} {/* Use current user's displayName */}
            </Text>
          </View>
      ...
      {loadingReviews ? (
              ...
              <View style={[styles.reviewerInfo, { flex: 1, marginRight: 8, marginBottom: 0 }]}>
                <Image
                  source={ // Priority: Live Profile (authorProfile) > Stored URL (userPhotoURL) > Default
                    review.authorProfile?.photoURL // If you fetch live profile data
                      ? { uri: review.authorProfile.photoURL }
                      : review.userPhotoURL // Photo URL stored with the review
                ...
                <View style={styles.reviewerDetails}>
                  <Text style={styles.reviewerName} numberOfLines={1} ellipsizeMode="tail">
                    {review.userName} {/* Name stored with the review */}
                  </Text>
        ...
        // Conditional "No reviews" messages
        !user ? (
          <Text style={styles.noReviews}>Login to see and submit reviews.</Text>
        ) : !isBought ? (
          <Text style={styles.noReviews}>Purchase the course to leave a review or see other reviews.</Text>
        ) : (
