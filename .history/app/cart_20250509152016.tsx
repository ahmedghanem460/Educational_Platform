...
import { useRouter } from "expo-router";
import { AntDesign, Ionicons, MaterialCommunityIcons } from '@expo/vector-icons'; // Added MaterialCommunityIcons
import { FIREBASE_DB, FIREBASE_AUTH } from "../config/FirebaseConfig"; // Added FIREBASE_AUTH
import { collection, getDocs, doc, deleteDoc, updateDoc, runTransaction } from 'firebase/firestore'; // Added updateDoc, runTransaction
import { onAuthStateChanged } from "firebase/auth"; // To listen to auth state

type CartItem = {
  id: string;          // Firestore document ID for the cart item
  productId: string;   // Original product ID
  name: string;
  price: number;       // Store price as a number if possible in Firestore, otherwise parse carefully
  image: string;
  quantity: number;
};

// Helper for showing toasts (cross-platform friendly)
const showFeedback = (message: string) => {
  if (Platform.OS === 'android') {
    ToastAndroid.show(message, ToastAndroid.SHORT);
  } else {
    Alert.alert(message); // Simple fallback for iOS
  }
};
...
const Cart = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(FIREBASE_AUTH, (user) => {
      setCurrentUser(user);
      if (!user) {
        // User logged out, clear cart and show appropriate message
        setCartItems([]);
        setTotalPrice(0);
        setLoading(false);
      }
