...
const CourseDetail = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(FIREBASE_AUTH, (user) => {
      setCurrentUser(user);
      if (user) {
        console.log("Auth state changed, user displayName:", user.displayName, "photoURL:", user.photoURL);
      } else {
  ...
  useEffect(() => {
    const checkCourseStatus = async () => {
      if (currentUser && title) {
        ...
        const courseRef = doc(FIREBASE_DB, 'users', currentUser.uid, 'courses', title);
        try {
    ...
    try {
      ...
      const reviewsData = await Promise.all(querySnapshot.docs.map(async (docSnapshot) => {
        ...
        const reviewItem: Review = {
          ...
          userPhotoURL: reviewDataFromDb.userPhotoURL,
        ...
        if (currentUser && reviewItem.userId === currentUser.uid) {
      ...
      if (currentUser && !foundUserReview && isUpdatingReview) { 
        setUserReview(''); 
        setRating(0); 
        setIsUpdatingReview(false); 
        setUserExistingReviewId(null);
      } else if (currentUser && !foundUserReview) {
        setUserReview('');
        setRating(0);
        setIsUpdatingReview(false);
        setUserExistingReviewId(null);
  ...
  const executePurchase = async () => {
    if (currentUser && title) {
      try {
        ...
        await setDoc(
          doc(FIREBASE_DB, 'users', currentUser.uid, 'courses', title),
          {
            title,
            description,
            price,
  ...
  const submitReview = async () => {
    ...
    try {
      const reviewDataPayload = {
        ...
        userPhotoURL: currentUser.photoURL || DEFAULT_AVATAR,
        rating,
