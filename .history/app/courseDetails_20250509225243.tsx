...
interface Review {
  ...
  userName: string;
  userEmail: string;
  userPhotoURL?: string; // Stored with the review
  rating: number;
  review: string;
  timestamp: Timestamp;
  authorProfile?: DocumentData; // Fetched from 'users' collection
}

const DEFAULT_AVATAR = 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y'; // Default Gravatar
...
const CourseDetail = () => {
  ...
  // State for managing user's own review editing
    ...
    const checkCourseStatus = async () => {
      if (user && title) {
        ...
        try {
          ...
          // If docSnap.exists() is false, isBought remains false (initial state unless changed by else block)
          setLoadingStatus(false);
        }
      } else {
        setIsBought(false); // User not logged in or title missing, cannot be bought
        setLoadingStatus(false);
      }
    };
    checkCourseStatus();
  ...
  useEffect(() => {
    fetchReviews();
  }, [title, user]); // Add user to dependency array
  ...
  const fetchReviews = async () => {
    ...
    // Do not reset userReview and rating here if user is typing a new review.
    // These will be overwritten if an existing review for the user is found.
    ...
    try {
      ...
      const reviewsData = await Promise.all(querySnapshot.docs.map(async (docSnapshot) => {
        ...
        const reviewItem: Review = {
          ...
          // Consolidate photoURL from new (userPhotoURL) or old (userProfile.photoURL object) structure
      ...
      if (user && !foundUserReview && isUpdatingReview) { 
        // User was in "update" mode, but their review is no longer found (e.g., deleted elsewhere)
        setUserReview(''); setRating(0); setIsUpdatingReview(false); setUserExistingReviewId(null);
      }
    } catch (error) {
  ...
  const submitReview = async () => {
    ...
    try {
      const reviewDataPayload = {
        ...
        userPhotoURL: user.photoURL || DEFAULT_AVATAR, // Store user's current photoURL
        rating,
      ...
      if (isUpdatingReview && userExistingReviewId) {
        ...
        await setDoc(reviewRef, reviewDataPayload, { merge: true }); // Use setDoc with merge for update
      ...
      // If it was a new submission, clear form fields.
      // For updates, fetchReviews will repopulate them with the (now same) updated data.
      if (!isUpdatingReview) { // If it was a brand new review
          setUserReview('');
          setRating(0);
      }
      // Refresh review list. This will also:
      // - Re-populate the form if it was an update.
      // - Find the new review and populate the form, turning it into an "update" form for subsequent edits.
      fetchReviews(); 
    } catch (error) {
        ...
        reviews.map((review) => (
          <View key={review.id} style={styles.reviewCard}>
            <View style={styles.reviewHeader}>
              <View style={styles.reviewerInfo}>
                <Image
                  source={ // Priority: Live Profile (authorProfile) > Stored URL (userPhotoURL) > Default
...
const styles = StyleSheet.create({
  ...
  reviewerInfo: {
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 15, // Adjusted this to match form's reviewerInfo style
  },
  reviewerAvatar: {
    width: 40,
    height: 40,
